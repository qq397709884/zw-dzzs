...
import cn.longicorn.modules.web.Servlets;
 ...
 *
 * 访问地址格式为(static-conetnt名称在servlet mapping中配置)：
...
public class StaticContentServlet extends HttpServlet {
    ...
    private static final long serialVersionUID = 537360645797298371L;
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // 高效读取文件内容并输出,然后关闭input file (内部使用的DEFAULT_BUFFER_SIZE = 1024 * 4)
        FileUtils.copyFile(contentInfo.file, output);
        /*
         * Normally you should not close the output stream. The servlet container will
		 * automatically close the stream after the servlet is finished running as part
		 * of the servlet request life-cycle.
		 * For instance, if you closed the stream it would not be available if you 
		 * implemented a filter.
		 * Having said all that, if you do close it nothing bad will happen as long
		 * as you don't try to use it again.
		 */
        output.flush();
    }
    ...
    private ContentInfo getContentInfo(String contentPath) {
        ...
        contentInfo.needGzip = contentInfo.length >= GZIP_MINI_LENGTH &&
